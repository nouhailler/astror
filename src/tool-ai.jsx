...
const AI_SUGGEST = [
  ...
  'Quels objets sont visibles depuis Paris à 23 h ?',
...
const AI_FALLBACK = {
  default: 'Connexion à l\'assistant indisponible dans cet aperçu. Une fois l\'application en ligne, je répondrai en temps réel selon votre ciel, votre position et votre matériel.',
...
export default function AiPage({ onBack }) {
  ...
  const prime = `Tu es Astror, un assistant expert en astronomie d'observation, intégré à une application mobile. L'utilisateur observe depuis ${profile.location || 'la France'}, son niveau est "${profile.level || 'amateur'}" et son matériel est : ${gear}. Réponds toujours en français, de façon concrète et pratique. Sois précis et concis (4 à 6 phrases), avec des données chiffrées utiles.`

  const [msgs, setMsgs] = useState([{ role: 'bot', text: `Bonsoir. Je connais votre ciel (${profile.location}) et votre matériel — demandez-moi quoi observer ce soir, comment trouver un objet, ou si les conditions sont bonnes.` }])
  ...
  async function send(text) {
    ...
    try {
      ...
      setMsgs(m => [...m.filter(x => !x.loading), { role: 'bot', text: pickFallback(q) }])
      ...
      <div className="tool-head">
        ...
        <div style={{ flex: 1 }}>
          ...
          <div className="meta" style={{ color: 'var(--good)', display: 'flex', alignItems: 'center', gap: 6, marginTop: 1 }}>
            <span className="dot pulse" style={{ background: 'var(--good)' }} /> IA · {profile.location}
          </div>
        </div>
      </div>
      ...
      <div style={{ flexShrink: 0, padding: '10px 16px calc(22px + var(--sab))', borderTop: '1px solid var(--line)', background: 'rgba(6,9,18,.7)' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 9, background: 'var(--surface-1)', border: '1px solid var(--line-2)', borderRadius: 999, padding: '5px 5px 5px 16px' }}>
          <input value={input} onChange={e => setInput(e.target.value)} onKeyDown={e => e.key === 'Enter' && send()}
            placeholder="Posez votre question…" style={{ flex: 1, background: 'none', border: 0, outline: 'none', color: 'var(--text)', fontSize: 14, fontFamily: 'var(--sans)' }} />
          <button onClick={() => send()} disabled={busy || !input.trim()} className="press" style={{ width: 40, height: 40, borderRadius: '50%', flexShrink: 0, border: 0,
